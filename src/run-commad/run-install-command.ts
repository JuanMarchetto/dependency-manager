    ...
    if (
      ...
      record.components = [
        ...record.components.filter(
          (item) => item.name !== record.components[found].name,
        ),
...
const runInstallCommand = (line: string, record: Record, splited: string[]) => {
  ...
  if (componentIndex >= 0) {
    if (record.components[componentIndex].status === status.INSTALLED) {
      installOutput.push(
        `${record.components[componentIndex].name} is already installed`,
      );
    } else {
      implicitInstall(record, componentIndex, installOutput);
      installOutput.push(
        `Installing ${record.components[componentIndex].name}`,
      );
      record.components = [
        ...record.components.filter(
          (item) => item.name !== record.components[componentIndex].name,
        ),
